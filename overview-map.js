...
function travelOverviewArtwork(days, source = {}, options = {}) {
  ...
  const multilineText = (lines, x, lineHeight = 31) => lines
    ...
    .join("");
  const arrowDefinitions = routeLayers.map((route) => `<marker id="overview-arrow-${route.day}" viewBox="0 0 24 24" refX="35" refY="12" markerWidth="30" markerHeight="30" markerUnits="userSpaceOnUse" orient="auto"><path d="M2 2 22 12 2 22z" fill="${esc(route.color)}" stroke="#fafaf4" stroke-width="1.2"/></marker>`).join("");
